...
// Načtení apartmánů z tabulky `properties`
$sql = "SELECT property_id AS id, name, address, gps_lat AS latitude, gps_lng AS longitude,
               calendar_id, status, number_of_beds, max_occupancy
        FROM properties
...
const propertyIconMap = {
  active:      'fa-bed',           // volno – lůžko
  inactive:    'fa-ban',           // neaktivní – přeškrtnutá značka
  maintenance: 'fa-tools'          // údržba – nářadí
};
...
function renderPropertyMarkers() {
  properties.forEach(prop => {
    // Vlastní filtr dle statusu (pokud chcete např. skrýt inactive)
    // if (prop.status !== 'active') return;

    const iconClass = propertyIconMap[prop.status] || 'fa-map-marker-alt';
    const customIcon = L.divIcon({
      html: `<i class="fas ${iconClass} fa-2x"></i>`,
      className: '',
      iconSize: [30,30]
    });

    // Popup s detaily + odkazy
    const popupHtml = `
      <div>
        <b>${prop.name}</b> (${prop.status.toUpperCase()})<br>
        📍 ${prop.address}<br>
        🛏️  Lůžka: ${prop.number_of_beds}, max. hostů: ${prop.max_occupancy}<br><br>
        <a href="https://calendar.google.com/calendar/u/0/r?cid=${encodeURIComponent(prop.calendar_id)}" target="_blank">
          📆 Zobrazit obsazenost
        </a><br>
        <a href="/booking.php?property_id=${prop.id}" class="reserve-btn">
          ➕ Rezervovat
        </a>
      </div>
    `;
...
function renderMarkers(filteredPlaces = null) {
  ...
  if (!selectedCategories.includes(place.category_id)) return;
    ...
    const statusLabel = isOpen ? '<span style="color: green; font-weight: bold;">🟢 Open</span>' : '<span style="color: red; font-weight: bold;">🔴 Closed</span>';
    let openingHtml = '';
...
renderMarkers();
renderPropertyMarkers();   // nově i apartmány
